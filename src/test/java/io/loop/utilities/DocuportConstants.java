package io.loop.utilities;

public class DocuportConstants {

    // url for the test env
    public static final String DOCUPORT_TEST = ConfigurationReader.getProperty("docuportBETA");
    public static final String EXPECTED_LOGO = "Docuport";
    public static final String EXPECTED_RESET_PASSWORD_HEADING = "Reset password";

    //user name for roles

    public static final String USERNAME_CLIENT = ConfigurationReader.getProperty("docuport.client.username");
    public static final String USERNAME_ADVISOR = ConfigurationReader.getProperty("docuport.advisor.username");
    public static final String USERNAME_SUPERVISOR = ConfigurationReader.getProperty("docuport.supervisor.username");
    public static final String USERNAME_EMPLOYEE = ConfigurationReader.getProperty("docuport.employee.username");
    public static final String PASSWORD = ConfigurationReader.getProperty("docuport.password");

    //messages
    public static final String ERROR_MESSAGE_FOR_EMPTY_PASSWORD = "Please enter your password";
    public static final String RESET_PASSWORD_MASSAGE = "Enter the email address associated with your account";
    public static final String EMAIL_SENT_MASSAGE = "We've sent you an email with a link to reset your password. Please check your email.";

    // forgot password URL
    public static final String FORGOT_PASSWORD_URL= "https://beta.docuport.app/reset-password";

    // email for reset password
    public static final String FORGOT_PASSWORD_EMAIL= "forgotpasswordg1@gmail.com";

    // roles
    public static String ADVISOR = "advisor";
    public static String CLIENT = "client";
    public static String SUPERVISOR = "supervisor";
    public static String EMPLOYEE = "employee";

    // waits
    public static final int EXTRA_SMALL = 3;
    public static final int SMALL = 5;
    public static final int MEDIUM = 7;
    public static final int LARGE = 10;
    public static final int EXTRA_LARGE = 20;
}

