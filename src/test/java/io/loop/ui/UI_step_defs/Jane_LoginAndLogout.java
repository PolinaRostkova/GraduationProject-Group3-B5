...
public class Jane_LoginAndLogout {
    ...
    public void user_logs_in_as(String role) throws InterruptedException {
        ...
        BrowserUtil.waitForClickablility(pages.getLoginPage().usernameInput, DocuportConstants.EXTRA_LARGE);
        ...
        BrowserUtil.waitForClickablility(pages.getLoginPage().passwordInput, DocuportConstants.EXTRA_LARGE);
        pages.getLoginPage().passwordInput.clear();
        pages.getLoginPage().passwordInput.sendKeys(pages.getLoginPage().getPasswordByRole(role));
        BrowserUtil.clickWithJS(pages.getLoginPage().loginButton);
    ...
    public void user_should_be_able_to_see_button_on_page(String button, String page) {
       switch (page.toLowerCase().trim()) {
           case "received docs" -> {
               BrowserUtil.waitForVisibility(pages.getLeftNavigatePage().receivedDocsButton, DocuportConstants.LARGE);
               assertTrue(pages.getLeftNavigatePage().receivedDocsButton.isDisplayed());
           }
           case "my uploads" -> {
               BrowserUtil.waitForVisibility(pages.getLeftNavigatePage().myUploads, DocuportConstants.LARGE);
    ...
    public void user_logs_out() {
        BrowserUtil.waitForClickablility(pages.getLogOutPage().userIcon, DocuportConstants.MEDIUM).click();
        LOG.info("User icon was clicked");
        BrowserUtil.waitForClickablility(pages.getLogOutPage().logOutButton, DocuportConstants.MEDIUM).click();
